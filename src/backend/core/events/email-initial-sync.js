...
export async function emailInitialSyncReceiver(kafkaMessage) {
  ...
  if (dbEmail) {
    if (dbEmail.users.includes(user.id)) {
    ...
    await collection.updateOne({ _id: email.id }, {
      $push: { users: user.id },
      $set: { [`userState.${user.id}`]: localUserState },
    });
    // add user in local array
    dbEmail.users.push(user.id);
...
const sendEmailNotification = async(user, dbEmail) => {
  const kafkaMessage = KafkaMessage.fromObject(user.id, {
    user,
  ...
  try {
    ...
    debug(`Sent notifiaction ${NOTIFICATION_NAME} for user "${user.email}" email "%s"`, dbEmail._id);
  } catch (e) {
